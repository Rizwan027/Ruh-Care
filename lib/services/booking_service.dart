    ...
    return _firestore
        ...
        .map((snapshot) {
          ...
          var bookings = snapshot.docs.map((doc) {
            ...
            if (booking.status == 'Upcoming' && booking.appointmentDateTime.isBefore(now)) {
              _firestore.collection('bookings').doc(booking.id).update({'status': 'Completed'});
  ...
  Stream<List<Booking>> getUpcomingBookings(String userId) {
    return _firestore
        ...
        .map((snapshot) {
          ...
          final validBookings = snapshot.docs.map((doc) {
            final data = doc.data();
            var booking = Booking.fromFirestore(data, doc.id);
            ...
            return booking;
          }).where((b) => b.status == 'Upcoming').toList();

          // Local sort by ascending time (closest upcoming first)
          validBookings.sort((a, b) => a.appointmentDateTime.compareTo(b.appointmentDateTime));
