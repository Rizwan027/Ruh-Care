...
class NotificationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Notifications', style: TextStyle(color: Color(0xFF2B4236), fontWeight: FontWeight.bold)),
        backgroundColor: const Color(0xFFF1F3EC),
        elevation: 0,
        ...
        : StreamBuilder<List<AppNotification>>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return const Center(child: CircularProgressIndicator(color: Color(0xFF2B4236)));
              }

              if (snapshot.hasError || !snapshot.hasData || snapshot.data!.isEmpty) {
                return const Center(
                  child: Text('No notifications available', style: TextStyle(color: Colors.grey, fontSize: 16)),
                );
              }
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return InkWell(
                    onTap: () async {
                      if (notification.message.toLowerCase().contains('bill')) {
                        // Redirect to the latest bill
                        final orderService = OrderService();
                        // We fetch the stream once and take the first list of orders
                        final orders = await orderService.getUserOrders(user.uid).first;
                              ...
                              builder: (context) => ReceiptScreen(
                                ...
                                items: [
                                  ReceiptItem(
                                    ...
                                    quantity: 1, // Quantity is already in the name string "1x ..."
                    ...
                    child: Container(
                      ...
                      decoration: const BoxDecoration(
                        border: Border(bottom: BorderSide(color: Color(0xFFE4E8D8))),
                      ),
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: const Color(0xFF2B4236).withOpacity(0.1),
                              shape: BoxShape.circle,
                            ),
                            child: const Icon(Icons.notifications, color: Color(0xFF2B4236), size: 20),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                 Text(notification.message, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 14, color: Color(0xFF2B4236))),
                                const SizedBox(height: 6),
                                  Text(
                                    DateFormat('MMM dd, yyyy • hh:mm a').format(notification.createdAt),
                                    style: const TextStyle(color: Color(0xFF6B8E67), fontSize: 12),
                                  ),
                              ],
                            ),
                          ),
